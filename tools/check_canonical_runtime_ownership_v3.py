...
workflow = (root / ".github/workflows/canonical-runtime-ownership-v3-gate.yml").read_text()
...
for path in (root / "scripts").rglob("*.gd"):
    ...
    if relative in ["scripts/world/world_binding_store_contract.gd", "scripts/world/versioned_snapshot_migration_service.gd"]:
        continue
    text = path.read_text()
    if '"runtime_power_feed"' in text and any(token in text for token in ["create_binding", "replace_binding", "upsert_binding"]):
...
checks = [
    ...
    ("static gate wired", "python tools/check_canonical_runtime_ownership_v3.py" in workflow),
