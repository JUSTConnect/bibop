...
func _run() -> void:
	...
	var hot: Dictionary = Evaluator.evaluate(_object("door", {"thermal_state":"overheated", "intent_state":"off", "operational_state":"locked"}), {"entity_contract":{"entity_subtype":"door", "capabilities":{"state":true, "health":true, "overheat":true, "power":true, "test_override":true}}})
	...
	_assert(str(Evaluator.evaluate(_object("door", {"state":"overheated"})).get("reason_code", "")) == "thermal.overheated", "legacy state=overheated maps to thermal_state")

	_assert(_result_value(Evaluator.evaluate(_object("fuse_box", {"state":"installed"})), "operational_state") == "installed", "fuse installed remains operational_state")
	_assert(_result_value(Evaluator.evaluate(_object("fuse_box", {"state":"empty"})), "operational_state") == "empty", "fuse empty remains operational_state")

	_assert(_result_value(Evaluator.evaluate(_object("power_cable", {"state":"connected", "broken":false})), "operational_state") == "connected", "cable connected remains operational_state")
	_assert(_result_value(Evaluator.evaluate(_object("power_cable", {"state":"disconnected", "broken":false})), "operational_state") == "disconnected", "cable disconnected remains operational_state")
	_assert(str(Evaluator.evaluate(_object("power_cable", {"state":"broken", "broken":false})).get("reason_code", "")) == "operational", "cable contract has no health capability, so legacy broken is not stored as health double truth")
	...
	_assert(_result_value(Evaluator.evaluate(_object("fuse", {"state":"disabled"})), "operational_state") == "disabled", "item disabled maps to operational_state")
	...
	var override_source: Dictionary = _object("door", {"test_override_values":{"intent_state":"off"}})
	...
	var forbidden_contract: Dictionary = {"entity_subtype":"door", "capabilities":{"state":true, "health":true, "overheat":false, "power":true, "test_override":false}}
