...
func _run() -> void:
	...
	_check(str(report.get("status", "")) == "blocked", "legacy blocked status changed")
	_check(str(report.get("decision_status", "")) == "blocked", "decision blocked status changed")
	var ready: Dictionary = Readiness.build([], {"serialization_ok":true, "write_ok":true, "loadable":true})
	_check(str(ready.get("status", "")) == "playable", "legacy playable status changed")
	_check(str(ready.get("decision_status", "")) == "ready", "new ready status missing")
	var warning_only: Dictionary = Readiness.build([issues[0]], {})
	_check(str(warning_only.get("status", "")) == "warning" and bool(warning_only.get("playable", false)), "warning compatibility state changed")
