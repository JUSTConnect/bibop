...
func _check_representative_shapes() -> void:
	...
	_expect_vec(Vector2(slab[1]["start"]), Vector2(87.84, 64.4), "slab accent start changed")
	_expect_vec(Vector2(slab[1]["end"]), Vector2(112.16, 64.4), "slab accent end changed")
...
func _check_wall_mounted_shapes() -> void:
	...
	_check_schema(door)
	_expect_eq(door.size(), 7, "door terminal outlined command count changed")
	_expect_eq(Rect2(door[5]["rect"]), Rect2(Vector2(15.0, 22.0), Vector2(10.0, 2.0)), "door terminal glow fill rect changed")
	_expect_eq(str(door[5]["kind"]), "rect", "door terminal glow fill kind changed")
	_expect_eq(bool(door[5]["filled"]), true, "door terminal glow fill must remain filled")
	_expect_eq(Rect2(door[6]["rect"]), Rect2(Vector2(15.0, 22.0), Vector2(10.0, 2.0)), "door terminal glow outline rect changed")
	_expect_eq(bool(door[6]["filled"]), false, "door terminal glow outline must remain unfilled")
