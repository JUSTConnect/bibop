...
func _base_entities() -> Array[Dictionary]:
	return [
		...
		_entity("duct_a", Vector2i(12, 0), "cooling", "external_air_duct", "cooling_system", ["mount", "side", "routing"])
	]
...
func _run() -> void:
	...
	var preserve_result: Dictionary = deletion_store.remove_object_by_id("terminal_a")
	_assert(bool(preserve_result.get("ok", false)), "default preserve deletion failed")
	_assert(deletion_store.has_binding("delete_preserve"), "default deletion removed relation")
	_assert(str(deletion_store.get_binding_by_id("delete_preserve").get("source_id", "")) == "terminal_a", "deleted endpoint was silently retargeted")
	...
	var cascade_result: Dictionary = cascade_store.remove_object_by_id("terminal_a", StoreRef.BINDING_POLICY_REMOVE_RELATED)
	_assert(bool(cascade_result.get("ok", false)), "remove-related deletion failed")
	_assert(not cascade_store.has_binding("delete_related"), "remove-related left relation behind")

	var serial_store = StoreRef.new()
	var serial_entities: Array[Dictionary] = _base_entities()
	serial_entities[2]["control_terminal_id"] = "terminal_a"
	serial_store.replace_snapshot(serial_entities)
	...
	var serialized_door: Dictionary = {}
	for entity_value in Array(snapshot.get("entities", [])):
		if entity_value is Dictionary and str(Dictionary(entity_value).get("id", "")) == "door_a":
			serialized_door = Dictionary(entity_value)
	_assert(not serialized_door.has("control_terminal_id"), "legacy logical field remained canonical serialized truth")
	...
	_assert(roundtrip_store.validate_consistency().is_empty(), "roundtrip consistency warnings: %s" % str(roundtrip_store.validate_consistency()))
	...
	var legacy_entities: Array[Dictionary] = [
		...
		_entity("legacy_reel", Vector2i(2, 1), "item", "power_cable_reel", "item", ["state", "routing"], {"endpoint_a_id": "legacy_terminal", "endpoint_b_id": "legacy_door", "path_cells": [Vector2i(0, 1)]}),
		_entity("legacy_duct", Vector2i(3, 1), "cooling", "external_air_duct", "cooling_system", ["mount", "side", "routing"], {"cooling_contour_member_ids": ["legacy_door"]})
	]
	var migration_store = StoreRef.new()
	var migration: Dictionary = migration_store.replace_serialized_snapshot({"format_version": 0, "objects": legacy_entities})
	_assert(bool(migration.get("ok", false)), "legacy snapshot migration failed: %s" % str(migration))
	_assert(bool(migration.get("legacy_bindings_migrated", false)), "legacy logical relation was not migrated")
	...
	var second_migration: Dictionary = migration_store.migrate_legacy_bindings(0)
	_assert(Array(Dictionary(second_migration.get("details", {})).get("created_binding_ids", [])).is_empty(), "legacy migration was not idempotent")
