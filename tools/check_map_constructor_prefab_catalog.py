...
normalize_text = normalize_body.group(0) if normalize_body is not None else ''
checks.append(('normalize_presentation_row exists', normalize_body is not None))
checks.append(('presentation catalog uses constructor prefab definitions', 'get_constructor_prefab_definition' in normalize_text))
checks.append(('presentation catalog uses constructor prefab schemas', 'get_constructor_prefab_property_schema' in normalize_text))
checks.append(('normalize writes configurable from constructor prefab metadata', 'normalized["configurable"]' in normalize_text))
checks.append(('normalize writes property schema from constructor prefab metadata', 'normalized["property_schema"]' in normalize_text))
checks.append(('presentation catalog does not use archetype definitions for configurability', 'get_archetype_definition' not in normalize_text))
checks.append(('presentation catalog does not use archetype schemas for constructor properties', 'get_archetype_property_schema' not in normalize_text))
