...
script_files = list((root / "scripts").rglob("*.gd"))
legacy_candidate_callers = []
for path in script_files:
    text = path.read_text()
    if "legacy_candidates(" in text:
...
for path in script_files:
    ...
    if relative in [
        "scripts/world/world_binding_store_contract.gd",
        "scripts/world/versioned_snapshot_migration_service.gd",
    ]:
        continue
    text = path.read_text()
    if '"runtime_power_feed"' in text and ("create_binding" in text or "replace_binding" in text or "upsert" in text):
...
for root_name in ["scripts/game", "scripts/world", "scripts/bipob"]:
    for path in (root / root_name).rglob("*.gd"):
        text = path.read_text()
        if "room_visual_renderer.gd" in text or "visual/renderer/object_renderer.gd" in text or "visual/renderer/route_renderer.gd" in text:
            renderer_importers.append(path.relative_to(root).as_posix())

power_assignments = []
for path, text in [
    ("scripts/world/power_system.gd", power),
    ("scripts/game/bipob_cable_runtime_service.gd", cable),
]:
    for field in legacy_source_fields:
        if re.search(rf'\["{re.escape(field)}"\]\s*=', text):
            power_assignments.append(f"{path}:{field}")

message_inference_patterns = [
    r'issue\.get\("message"[^\n]*\.begins_with\(',
    r'issue\.get\("message"[^\n]*\.contains\(',
    r'issue\.get\("message"[^\n]*\.ends_with\(',
    r'message[^\n]*\.begins_with\(',
...
message_inference = []
for pattern in message_inference_patterns:
...
checks = [
    ...
    ("power runtime writes resolved source", "resolved_source_id" in power and "resolved_circuit_id" in power and "resolved_source_id" in cable and "resolved_circuit_id" in cable),
    ...
    ("static gate wired", "python tools/check_canonical_runtime_ownership.py" in workflow),
