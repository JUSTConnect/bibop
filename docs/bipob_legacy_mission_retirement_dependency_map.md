...
- `get_task_test_mission_id()` keeps `mission_10` as the compatibility mission id, while `get_task_test_layout_id()` names the neutral `task_test` layout/catalog alias so UI callers do not depend on raw mission index `10`. `get_task_test_source_id()` is the canonical source-id boundary for new Map Constructor persistence/export metadata and returns `task_test`.
...
- `MissionManager.get_task_test_mission_id()`, `get_task_test_layout_id()`, `get_task_test_source_id()`, and `is_task_test_mission_id()` keep `task_test` canonical for new source metadata while accepting both `task_test` and `mission_10` for compatibility. This PR intentionally does not remove or rename `mission_10` resources.
