...
const RESULT_CODES: Array[String] = ["valid", "missing", "source_missing", "target_missing", "wrong_type", "inactive", "capacity_exceeded", "duplicate", "cycle", "unsupported_role", "physical_relation_forbidden", "invalid_format_version", "binding_cleanup_required"]
...
const ROLE_REGISTRY: Dictionary = {
	ROLE_CONTROL_TERMINAL: {"source_groups":["terminal"], "source_types":["terminal", "information_terminal", "control_terminal"], "target_capability":"control", "max_per_target":1, "acyclic":true},
	ROLE_ACCESS_TERMINAL: {"source_groups":["terminal"], "source_types":["terminal", "information_terminal", "access_terminal"], "target_capability":"access", "max_per_target":1, "acyclic":true},
	ROLE_ACCESS_ITEM: {"source_groups":["item"], "source_entity_types":["item"], "target_capability":"access", "acyclic":true},
	ROLE_PREFERRED_POWER_SOURCE: {"source_capability":"power", "target_types":["power_source", "power_source_class_1", "power_source_class_2", "power_source_class_3"], "target_roles":["power_source"], "max_per_source":1, "acyclic":true},
	ROLE_LIGHT_CONTROLLER: {"source_capability":"control", "source_groups":["terminal"], "source_types":["terminal", "information_terminal", "light_switcher", "power_switcher", "light_switch"], "target_entity_types":["light"], "target_groups":["lighting", "light"], "target_types":["light", "wall_light", "light_source"], "max_per_target":1, "acyclic":true},
	ROLE_PLATFORM_CONTROLLER: {"source_capability":"control", "source_groups":["terminal"], "source_types":["terminal", "information_terminal", "platform_controller", "power_switcher"], "target_groups":["platform"], "target_types":["platform", "rotating_platform", "lifting_platform"], "max_per_target":1, "acyclic":true}
}

const PHYSICAL_RELATION_ROLES: Array[String] = ["power_cable", "power_cable_segment", "runtime_power_feed", "power_cable_reel", "cable_endpoint", "passive_route", "duct_adjacency", "pipe_adjacency"]
const PHYSICAL_PARAMETER_FIELDS: Array[String] = ["end_1", "end_2", "path_cells", "endpoint_a_id", "endpoint_b_id", "socket_id", "cable_path_cells", "route_cells", "topology_neighbors"]
const PHYSICAL_OBJECT_TYPES: Array[String] = ["power_cable", "power_cable_reel", "external_air_duct", "external_water_pipe", "air_duct", "water_pipe"]
const LEGACY_LOGICAL_LINK_FIELDS: Array[String] = ["control_terminal_id", "linked_terminal_id", "linked_terminal_ids", "required_terminal_id", "required_key_id", "required_digital_key_id", "required_access_code_id", "preferred_source_id", "linked_light_ids", "target_light_ids", "linked_platform_ids", "linked_door_id", "linked_door_ids", "target_door_id", "connected_device_ids"]
...
static func make_result(code: String, binding: Dictionary = {}, details: Dictionary = {}, success_override: Variant = null) -> Dictionary:
	...
	return {"ok":success, "success":success, "code":code, "reason_code":code, "binding_id":str(binding.get("id", "")), "source_id":str(binding.get("source_id", "")), "target_id":str(binding.get("target_id", "")), "role":str(binding.get("role", "")), "details":details.duplicate(true)}
...
static func canonicalize_record(record: Dictionary) -> Dictionary:
	...
	return {"id":str(record.get("id", "")).strip_edges(), "role":str(record.get("role", "")).strip_edges().to_lower(), "source_id":str(record.get("source_id", "")).strip_edges(), "target_id":str(record.get("target_id", "")).strip_edges(), "parameters":parameters, "format_version":int(record.get("format_version", FORMAT_VERSION))}
...
static func validate_record(record: Dictionary, entities_by_id: Dictionary, bindings_by_id: Dictionary, replacing_binding_id: String = "", allow_missing_entities: bool = false) -> Dictionary:
	...
	if not ROLE_REGISTRY.has(role):
		return make_result("physical_relation_forbidden" if PHYSICAL_RELATION_ROLES.has(role) else "unsupported_role", binding, {"role":role})
	for field_name in PHYSICAL_PARAMETER_FIELDS:
		if Dictionary(binding.get("parameters", {})).has(field_name):
	...
	for existing_id_value in bindings_by_id.keys():
		...
		if existing_id != replacing_binding_id and _relation_key(Dictionary(bindings_by_id[existing_id])) == _relation_key(binding):
	...
	if not _entity_matches(source, descriptor, "source") or not _entity_matches(target, descriptor, "target"):
		return make_result("wrong_type", binding, {"source_group":_object_group(source), "source_type":_object_type(source), "target_group":_object_group(target), "target_type":_object_type(target)})
	if _entity_is_inactive(source) or _entity_is_inactive(target):
		return make_result("inactive", binding, {"source_inactive":_entity_is_inactive(source), "target_inactive":_entity_is_inactive(target)})
...
static func build_state(records: Array[Dictionary], entities_by_id: Dictionary, preserve_semantic_invalid: bool = true) -> Dictionary:
	...
	for binding in sorted_records:
		...
		var preservable: bool = bool(Dictionary(validation.get("details", {})).get("preservable", false)) or (preserve_semantic_invalid and code in ["source_missing", "target_missing", "wrong_type", "inactive", "capacity_exceeded", "cycle"])
		if code != VALID_CODE and not preservable:
			return {"ok":false, "success":false, "code":code, "reason_code":code, "binding":binding.duplicate(true), "diagnostics":diagnostics.duplicate(true)}
		bindings_by_id[str(binding.get("id", ""))] = binding.duplicate(true)
		if code != VALID_CODE:
			diagnostics.append(validation.duplicate(true))
	return {"ok":true, "success":true, "code":VALID_CODE, "reason_code":VALID_CODE, "bindings_by_id":bindings_by_id, "indexes":rebuild_indexes(bindings_by_id), "diagnostics":diagnostics}
...
static func rebuild_indexes(bindings_by_id: Dictionary) -> Dictionary:
	...
	return {"by_source":by_source, "by_target":by_target, "by_role":by_role}
...
static func validate_indexes(bindings_by_id: Dictionary, by_source: Dictionary, by_target: Dictionary, by_role: Dictionary) -> Array[String]:
	...
	if var_to_str(Dictionary(expected.get("by_source", {}))) != var_to_str(by_source): warnings.append("binding_source_index_mismatch")
	if var_to_str(Dictionary(expected.get("by_target", {}))) != var_to_str(by_target): warnings.append("binding_target_index_mismatch")
	if var_to_str(Dictionary(expected.get("by_role", {}))) != var_to_str(by_role): warnings.append("binding_role_index_mismatch")
	return warnings
...
static func legacy_candidates(entities_by_id: Dictionary) -> Array[Dictionary]:
	...
	for entity_id_value in entity_ids:
		...
		_append_legacy_door_links(candidates, entity, entity_id)
...
static func _append_legacy_door_links(candidates: Array[Dictionary], entity: Dictionary, entity_id: String) -> void:
	if _object_group(entity) == "terminal" or _has_capability(entity, "control"):
		_append_legacy_scalar(candidates, entity, "linked_door_id", ROLE_CONTROL_TERMINAL, false, entity_id)
		_append_legacy_scalar(candidates, entity, "target_door_id", ROLE_CONTROL_TERMINAL, false, entity_id)
		_append_legacy_array(candidates, entity, "linked_door_ids", ROLE_CONTROL_TERMINAL, entity_id)
	if _entity_type(entity) == "item" or _object_group(entity) == "item":
		_append_legacy_scalar(candidates, entity, "linked_door_id", ROLE_ACCESS_ITEM, false, entity_id)
		_append_legacy_scalar(candidates, entity, "target_door_id", ROLE_ACCESS_ITEM, false, entity_id)
		_append_legacy_array(candidates, entity, "linked_door_ids", ROLE_ACCESS_ITEM, entity_id)
...
static func _append_legacy_scalar(candidates: Array[Dictionary], entity: Dictionary, field_name: String, role: String, value_is_source: bool, entity_id: String) -> void:
	...
	if related_id.is_empty(): return
	candidates.append(_legacy_record(role, related_id if value_is_source else entity_id, entity_id if value_is_source else related_id, field_name))
...
static func _append_legacy_array(candidates: Array[Dictionary], entity: Dictionary, field_name: String, role: String, source_id: String) -> void:
	...
	for target_id_value in target_ids:
		...
		if not target_id.is_empty(): candidates.append(_legacy_record(role, source_id, target_id, field_name))

static func _legacy_record(role: String, source_id: String, target_id: String, field_name: String) -> Dictionary:
	return {"id":"legacy__%s__%s__%s" % [_id_token(role), _id_token(source_id), _id_token(target_id)], "role":role, "source_id":source_id, "target_id":target_id, "parameters":{"migrated_from_field":field_name}, "format_version":FORMAT_VERSION}
...
static func _deduplicate_candidates(candidates: Array[Dictionary]) -> Array[Dictionary]:
	...
	for candidate in candidates:
		...
		if not by_relation.has(key): by_relation[key] = candidate.duplicate(true)
...
static func _entity_matches(entity: Dictionary, descriptor: Dictionary, prefix: String) -> bool:
	...
	if groups.is_empty() and types.is_empty() and entity_types.is_empty() and roles.is_empty() and capability.is_empty(): return true
	return (not capability.is_empty() and _has_capability(entity, capability)) or groups.has(_object_group(entity)) or types.has(_object_type(entity)) or entity_types.has(_entity_type(entity)) or roles.has(str(entity.get("generic_power_role", "")).strip_edges().to_lower())
...
static func _entity_is_inactive(entity: Dictionary) -> bool:
	return (entity.has("binding_active") and not bool(entity.get("binding_active", true))) or (entity.has("is_operational") and not bool(entity.get("is_operational", true)))

static func _is_physical_topology_object(entity: Dictionary) -> bool:
	if PHYSICAL_OBJECT_TYPES.has(_object_type(entity)): return true
	return str(entity.get("generic_power_role", "")).strip_edges().to_lower() in ["cable_link", "cable_endpoint", "airflow_path_cell", "water_route_cell"]
...
static func _count_role_endpoint(bindings_by_id: Dictionary, role: String, endpoint_field: String, endpoint_id: String, replacing_binding_id: String) -> int:
	...
	for binding_id_value in bindings_by_id.keys():
		var binding_id: String = str(binding_id_value)
		if binding_id != replacing_binding_id and str(Dictionary(bindings_by_id[binding_id]).get("role", "")) == role and str(Dictionary(bindings_by_id[binding_id]).get(endpoint_field, "")) == endpoint_id:
			count += 1
	return count
...
static func _would_create_cycle(bindings_by_id: Dictionary, candidate: Dictionary, replacing_binding_id: String) -> bool:
	...
	for binding_id_value in bindings_by_id.keys():
		var binding_id: String = str(binding_id_value)
		if binding_id == replacing_binding_id: continue
		var binding: Dictionary = Dictionary(bindings_by_id[binding_id])
		if str(binding.get("role", "")) == role: _append_index(adjacency, str(binding.get("source_id", "")), str(binding.get("target_id", "")))
	_append_index(adjacency, str(candidate.get("source_id", "")), str(candidate.get("target_id", "")))
	return _path_exists(adjacency, str(candidate.get("target_id", "")), str(candidate.get("source_id", "")), {})

static func _path_exists(adjacency: Dictionary, current_id: String, expected_id: String, visited: Dictionary) -> bool:
	if current_id == expected_id: return true
	if visited.has(current_id): return false
	visited[current_id] = true
	for next_id_value in Array(adjacency.get(current_id, [])):
		if _path_exists(adjacency, str(next_id_value), expected_id, visited): return true
	return false

static func _append_index(index: Dictionary, key: String, value: String) -> void:
	if key.is_empty() or value.is_empty(): return
...
static func _id_token(value: String) -> String:
	...
	for index in range(value.length()):
		...
		result += character if character == "_" or character.is_valid_identifier() or character.is_valid_int() else "_"
	while result.begins_with("_"): result = result.trim_prefix("_")
	while result.ends_with("_"): result = result.trim_suffix("_")
	return result
