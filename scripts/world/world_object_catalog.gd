...
static func normalize_archetype_object(object_data: Dictionary) -> Dictionary:
	...
	for metadata_field in ["placement_mode", "blocks_movement", "blocks_vision", "configurable", "replaces_tile_with"]:
		if definition.has(metadata_field):
			data[metadata_field] = definition[metadata_field]
...
static func create_archetype_object(archetype_id: String, id_override: String = "", overrides: Dictionary = {}) -> Dictionary:
	...
	var data: Dictionary = {}
	if OBJECT_LIBRARY.has(runtime_type):
		data = create_world_object(runtime_type, id_override)
	else:
		var object_id: String = id_override if not id_override.is_empty() else "%s_%s" % [runtime_type, str(Time.get_unix_time_from_system())]
