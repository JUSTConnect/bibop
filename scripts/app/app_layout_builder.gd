...
# AppLayoutBuilder
# Создаёт layout и возвращает ссылки на UI-ноды. Не хранит gameplay state.
...
static func _build_header(callbacks: Dictionary) -> Control:
	...
	row.add_child(title)
	row.add_child(_make_button("Reload / Clear", 112, callbacks.get("reload", Callable())))
	return panel
...
static func _build_map_panel(callbacks: Dictionary) -> Dictionary:
	...
	tool_label.custom_minimum_size = Vector2(82, 0)
	tool_label.clip_text = true
	toolbar.add_child(tool_label)
	toolbar.add_child(_make_button("Place", 58, callbacks.get("place", Callable())))
	toolbar.add_child(_make_button("Erase", 58, callbacks.get("erase", Callable())))
	toolbar.add_child(_make_button("Use", 58, callbacks.get("use", Callable())))
	toolbar.add_child(_make_button("Clear", 58, callbacks.get("clear", Callable())))
	toolbar.add_child(_make_button("Save", 58, callbacks.get("save", Callable())))
	toolbar.add_child(_make_button("Load", 58, callbacks.get("load", Callable())))
	...
	map_canvas.connect("cell_pressed", callbacks.get("cell_pressed", Callable()))
	stack.add_child(map_canvas)
	return {"panel": panel, "tool_mode_label": tool_label, "map_canvas": map_canvas}
