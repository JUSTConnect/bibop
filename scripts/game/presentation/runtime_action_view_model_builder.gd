extends RefCounted
class_name RuntimeActionViewModelBuilder
...
static func should_hide_action(controller: Variant, world_object: Dictionary, action_id: String) -> bool:
	if action_id.strip_edges().to_lower() != "cut":
		return false
	...
	for field_name in ["object_type", "type", "archetype_id", "map_constructor_prefab_id", "prefab_id"]:
		if str(world_object.get(field_name, "")).strip_edges().to_lower() in ["power_cable", "cable", "cable_reel", "power_cable_reel"]:
			return true
	return false

static func build(controller: Variant, target_object: Dictionary, target_position: Vector2i) -> Dictionary:
	var target: Dictionary = WorldObjectCatalogRef.normalize_world_object_contract(target_object)
	if BreachableWallServiceRef.is_breachable_wall_data(target):
		target = BreachableWallServiceRef.normalize_runtime_breachable_wall_data(target)
	if str(target.get("object_group", "")) == "door":
		target = WorldObjectCatalogRef.normalize_door_state_fields(WorldObjectCatalogRef.normalize_door_contract(target))
	if _is_platform(target) and not _platform_contains_actor(controller, target, target_position):
	...
	for value in raw_ids:
		...
		if BreachableWallServiceRef.is_breachable_wall_data(target) and action_id != BreachableWallServiceRef.ACTION_BREAK_BREACHABLE_WALL:
			continue
		if not action_id.is_empty() and not should_hide_action(controller, target, action_id) and not action_ids.has(action_id):
	...
	if BreachableWallServiceRef.is_active_breachable_wall_data(target) and not action_ids.has(BreachableWallServiceRef.ACTION_BREAK_BREACHABLE_WALL):
		if bool(_breach_payload(controller, target, target_position).get("show_heavy_claw", false)):
	...
	if str(target.get("object_group", "")) == "door":
		...
		if not expected.is_empty() and not action_ids.has(expected):
			action_ids.append(expected)
	...
	for action_id in action_ids:
		...
		var gate: Dictionary = InteractionSystemRef.can_apply_action(actor, module, target, action_id)
		...
		descriptors.append(_descriptor(action_id, label, available, reason_code, requirements, target, target_position, module, gate))
	...
	return {
		...
		"unfiltered_action_ids":raw_ids,
		"available_action_ids":available_ids,
		"primary_action_id":str(primary.get("action_code", "")),
		"primary_action_label":str(primary.get("label", "Action")),
		"has_available_action":not available_ids.is_empty(),
		"has_interaction_target":not target.is_empty() and not descriptors.is_empty(),
		"disabled_reason":str(primary.get("reason_code", "target_missing" if target.is_empty() else "no_available_action"))
	}

static func _descriptor(code: String, label: String, available: bool, reason_code: String, requirements: Array, target: Dictionary, target_cell: Vector2i, module: Dictionary, gate: Dictionary) -> Dictionary:
	return {
		"action_code":code,
		"label_key":"action.%s.label" % code,
		"available":available,
		"reason_code":reason_code,
		"requirements":RequirementCatalogRef.deduplicate(requirements),
		"target_id":str(target.get("id", "")),
		"context":{"target_type":str(target.get("object_type", target.get("object_group", ""))), "target_cell":target_cell},
		"id":code,
		"label":label,
		"enabled":available,
		"reason":reason_code,
