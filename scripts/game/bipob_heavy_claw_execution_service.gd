...
static func execute_heavy_claw_action(controller: Variant, world_object: Dictionary, target_position: Vector2i, action_id: String) -> Dictionary:
	...
	var move_result: Dictionary = Dictionary(controller.mission_manager.move_world_object_by_heavy_claw(String(world_object.get("id", "")), target_destination))
	...
	if success:
		...
		return _build_result(true, true, String(move_result.get("message", "Moved object.")), true, true, true, true, true, true, "ok")
	return _build_result(true, false, String(move_result.get("message", "Cannot move object there.")), false, false, false, false, true, false, "move_unavailable")
