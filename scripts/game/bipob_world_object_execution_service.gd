...
static func execute_world_object_action(controller: Variant, world_object: Dictionary, target_position: Vector2i, actor: Dictionary, module: Dictionary, action_id: String) -> Dictionary:
	...
	if not bool(action_result.get("success", false)):
		return _build_result(false, String(action_result.get("message", "Action failed.")), world_object, target_position, action_result, "action_failed")
	...
	if action_id == "unlock" and String(world_object.get("object_group", "")) == "door" and WorldObjectCatalogRef.normalize_access_type(world_object.get("access_type", world_object.get("lock_type", ""))) == WorldObjectCatalogRef.ACCESS_TYPE_KEY_CARD and controller.mission_manager.has_method("remove_keycard_if_no_door_references"):
		controller.mission_manager.call("remove_keycard_if_no_door_references", String(world_object.get("required_key_id", "")))
	_apply_explicit_power_event(controller, world_object, action_id, action_result)
	var action_message: String = String(action_result.get("message", "Action complete."))
	if String(world_object.get("object_group", "")) != "door" or action_id not in ["open", "close", "unlock"]:
...
static func _apply_explicit_power_event(controller: Variant, world_object: Dictionary, action_id: String, action_result: Dictionary) -> void:
	if action_id == "switch":
		var object_type: String = String(world_object.get("object_type", "")).strip_edges().to_lower()
		...
		if object_type in ["light_switch", "circuit_switch", "circuit_breaker"]:
			...
			if controller.mission_manager.has_method("_get_power_event_filter_for_object"):
				power_filter = String(controller.mission_manager.call("_get_power_event_filter_for_object", world_object))
