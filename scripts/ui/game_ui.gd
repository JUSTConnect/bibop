...
func _map_constructor_prefab_matches_filters(entry: Dictionary) -> bool:
	...
	var prefab_id: String = _safe_ui_string(entry.get("id", ""))
	var label_text: String = _safe_ui_string(entry.get("display_name", entry.get("label", "")))
	var category_text: String = _safe_ui_string(entry.get("category", ""))
	var placement_mode: String = _safe_ui_string(entry.get("placement_mode", "")).to_lower()
	var roles_text: String = _format_map_constructor_variant_list(entry.get("system_roles", []))
	var tags_text: String = _format_map_constructor_variant_list(entry.get("tags", []))
	var description: String = _safe_ui_string(entry.get("description", ""))
	if map_constructor_prefab_category_filter != "All" and category_text != map_constructor_prefab_category_filter:
		return false
	if map_constructor_prefab_role_filter != "All" and not _get_map_constructor_variant_string_list(entry.get("system_roles", [])).has(map_constructor_prefab_role_filter):
		return false
...
func _get_map_constructor_prefab_group_name(entry: Dictionary) -> String:
	var prefab_id: String = _safe_ui_string(entry.get("id", ""))
	var category_text: String = _safe_ui_string(entry.get("category", ""))
	var placement_mode: String = _safe_ui_string(entry.get("placement_mode", ""))
	if placement_mode == "wall_mounted":
		return "Wall-mounted"
...
func _select_map_constructor_prefab(prefab_id: String) -> void:
	...
	if mission_manager_runtime != null and mission_manager_runtime.has_method("get_map_constructor_prefab_metadata"):
		var metadata_result: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("get_map_constructor_prefab_metadata", prefab_id))
		var metadata: Dictionary = _safe_ui_dictionary(metadata_result.get("prefab", {}))
		...
		var catalog_entries: Array = _safe_ui_array(mission_manager_runtime.call("get_map_constructor_prefab_catalog"))
		for entry_variant in catalog_entries:
			...
			if _safe_ui_string(entry.get("placement_mode", "")).to_lower() == "wall_mounted":
				selected_map_constructor_mounting_mode = "wall_mounted"
			break
	var wall_mounted_prefab_defaults: Dictionary = {"light": true, "light_switch": true, "fuse_box": true, "circuit_breaker": true, "power_cable_reel": true}
...
func _get_map_constructor_variant_string_list(value: Variant) -> Array[String]:
	...
	if value is Array or value is PackedStringArray:
		for item: Variant in value:
			var item_text: String = _safe_ui_string(item).strip_edges()
			if not item_text.is_empty():
				items.append(item_text)
		return items
	var scalar_text: String = _safe_ui_string(value).strip_edges()
...
func _format_map_constructor_variant_list(value: Variant, separator: String = ", ") -> String:
	return separator.join(_get_map_constructor_variant_string_list(value))
...
func _get_map_constructor_prefab_preview_kind(entry: Dictionary) -> String:
	var prefab_id: String = _safe_ui_string(entry.get("id", "")).to_lower()
	var category_text: String = _safe_ui_string(entry.get("category", "")).to_lower()
	var placement_mode: String = _safe_ui_string(entry.get("placement_mode", "")).to_lower()
	var tags_text: String = _format_map_constructor_variant_list(entry.get("tags", []), " ").to_lower()
	var roles_text: String = _format_map_constructor_variant_list(entry.get("system_roles", []), " ").to_lower()
...
func _format_map_constructor_prefab_placeability(entry: Dictionary) -> Dictionary:
	...
	if entry.has("placeability"):
		var placeability: Dictionary = _get_map_constructor_prefab_placeability(entry)
		if bool(placeability.get("ok", false)):
			return {"text":"Placeable", "role":"ok"}
		var message: String = _safe_ui_string(placeability.get("message", "Not placeable here."), "Not placeable here.").strip_edges()
		if message.is_empty():
			message = "Not placeable here."
		return {"text":"Not placeable here — %s" % message, "role":"danger"}
	var placement_hint: String = _safe_ui_string(entry.get("placement_hint", "")).strip_edges()
...
func _create_map_constructor_prefab_preview(entry: Dictionary) -> Control:
	...
	var symbol_text: String = _safe_ui_string(entry.get("preview_symbol", entry.get("icon_symbol", ""))).strip_edges()
	if symbol_text.is_empty():
		symbol_text = _get_map_constructor_prefab_preview_symbol(preview_kind)
	...
	kind_label.text = _get_map_constructor_prefab_preview_label(preview_kind)
	...
	kind_label.add_theme_color_override("font_color", UI_COLOR_TEXT)
	...
	mode_label.add_theme_color_override("font_color", UI_COLOR_TEXT_DIM)
...
func _create_map_constructor_prefab_card(entry: Dictionary) -> Button:
	...
	var category_text: String = _safe_ui_string(entry.get("category", "Uncategorized"), "Uncategorized")
	var placement_mode: String = _safe_ui_string(entry.get("placement_mode", "unknown"), "unknown")
	var roles_text: String = _format_map_constructor_variant_list(entry.get("system_roles", []))
	...
	var description: String = _safe_ui_string(entry.get("description", "")).strip_edges()
	...
	if map_constructor_prefab_show_diagnostics:
		...
		if entry.has("placeability"):
			var placeability: Dictionary = _get_map_constructor_prefab_placeability(entry)
			var reason: String = _safe_ui_string(placeability.get("reason", "")).strip_edges()
...
func _add_map_constructor_description_editor(section: VBoxContainer, data: Dictionary, entity_kind: String, entity_id: String) -> void:
	...
	var apply_description := func() -> void:
		if mission_manager_runtime == null or not mission_manager_runtime.has_method("update_map_constructor_entity_properties"):
			return
		var result: Dictionary = mission_manager_runtime.call("update_map_constructor_entity_properties", entity_kind, entity_id, {"description": desc_edit.text})
		show_hint(_safe_ui_string(result.get("message", "Description updated."), "Description updated."))
		_refresh_map_constructor_panels()
		if field_runtime != null and field_runtime.has_method("request_visual_refresh"):
			field_runtime.call("request_visual_refresh")
	...
	section.add_child(_create_property_row("Description", desc_edit))
...
func _create_map_constructor_description_block(data: Dictionary, entity_kind: String, entity_id: String) -> Control:
	...
	_add_map_constructor_description_editor(section, data, entity_kind, entity_id)
	return section
...
func _add_bool_property(section: VBoxContainer, label: String, entity_kind: String, entity_id: String, field_name: String, current_value: Variant) -> void:
	...
	section.add_child(_create_property_row(label, check))

func _add_preset_buttons(section: VBoxContainer, entity_kind: String, entity_id: String) -> void:
	if mission_manager_runtime == null or not mission_manager_runtime.has_method("get_map_constructor_property_presets"):
		return
	var row: HFlowContainer = HFlowContainer.new()
	for preset in _safe_ui_array(mission_manager_runtime.call("get_map_constructor_property_presets", entity_kind, entity_id)):
		var preset_data: Dictionary = _safe_ui_dictionary(preset)
		...
		button.pressed.connect(func() -> void:
			var result: Dictionary = mission_manager_runtime.call("apply_map_constructor_property_preset", entity_kind, entity_id, preset_id)
...
func _add_link_picker(section: VBoxContainer, entity_kind: String, entity_id: String, link_type: String, title: String) -> void:
	...
	jump_button.pressed.connect(func() -> void:
		if current_target.is_empty() or mission_manager_runtime == null:
			return
		var target_entity: Dictionary = mission_manager_runtime.call("get_map_constructor_entity_by_id", "world_object", current_target)
		if bool(target_entity.get("ok", false)):
...
func _add_key_door_link_section(parent: VBoxContainer, entity_kind: String, entity_id: String, data: Dictionary) -> void:
	if mission_manager_runtime == null or not mission_manager_runtime.has_method("get_map_constructor_key_door_link_candidates"):
		return
	var section := _create_inspector_section("Door Link")
	var current_id: String = _safe_ui_string(data.get("linked_door_id", "")).strip_edges()
	...
	if doors.is_empty():
		...
		none_label.text = _safe_ui_string(candidates.get("message", "No compatible doors placed on the map."))
		section.add_child(none_label)
	else:
		...
		if option.selected < 0 and option.item_count > 0:
			option.select(0)
		section.add_child(_create_property_row("Door", option))
		var link_button := Button.new()
		link_button.text = "Link Selected Door"
		link_button.pressed.connect(func() -> void:
			var door_id: String = _safe_ui_string(option.get_selected_metadata())
			var result: Dictionary = mission_manager_runtime.call("set_map_constructor_entity_link", entity_kind, entity_id, "key_door", door_id)
			show_hint(_safe_ui_string(result.get("message", "Door linked."), "Door linked."))
			_refresh_map_constructor_panels()
			if field_runtime != null and field_runtime.has_method("request_visual_refresh"):
				field_runtime.call("request_visual_refresh")
			_show_map_constructor_inspector(selected_map_constructor_entity_cell, selected_map_constructor_entity_kind, selected_map_constructor_entity_id)
	...
	if not current_id.is_empty():
		...
		unlink_button.pressed.connect(func() -> void:
			var result: Dictionary = mission_manager_runtime.call("set_map_constructor_entity_link", entity_kind, entity_id, "key_door", "")
			show_hint(_safe_ui_string(result.get("message", "Door unlinked."), "Door unlinked."))
			_clear_map_constructor_link_target()
			_refresh_map_constructor_panels()
			if field_runtime != null and field_runtime.has_method("request_visual_refresh"):
				field_runtime.call("request_visual_refresh")
...
func _add_door_linked_key_section(parent: VBoxContainer, entity_id: String, data: Dictionary) -> void:
	var section := _create_inspector_section("Linked Key")
	var key_id: String = _safe_ui_string(data.get("required_key_id", "")).strip_edges()
	var label := Label.new()
	label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	if key_id.is_empty():
...
func _show_map_constructor_inspector(cell: Vector2i, preferred_entity_kind: String = "", preferred_entity_id: String = "") -> void:
	...
	if runtime_map_constructor_inspector_scroll != null and is_instance_valid(runtime_map_constructor_inspector_scroll):
		preserve_scroll_value = runtime_map_constructor_inspector_scroll.scroll_vertical
	if runtime_map_constructor_inspector_panel != null and is_instance_valid(runtime_map_constructor_inspector_panel):
		runtime_map_constructor_inspector_panel.queue_free()
	runtime_map_constructor_inspector_panel = null
	runtime_map_constructor_inspector_scroll = null
	...
	header_label.text = "Selected %s: %s" % [entity_kind, _safe_ui_string(data.get("display_name", entity_id), entity_id)]
	...
	scroll.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	runtime_map_constructor_inspector_scroll = scroll
	...
	scroll.add_child(v)
	var identity := _create_inspector_section("1. Object Identity")
	var id_label: Label = Label.new(); id_label.text = entity_id; identity.add_child(_create_property_row("ID", id_label))
	_add_text_property(identity, "Name", entity_kind, entity_id, "display_name", data.get("display_name", ""))
	_add_map_constructor_description_editor(identity, data, entity_kind, entity_id)
	var type_label: Label = Label.new(); type_label.text = _safe_ui_string(data.get("object_type", data.get("item_type", "item")), "item"); identity.add_child(_create_property_row("Object type", type_label))
	...
	for class_field in ["object_class", "door_class", "terminal_class", "power_source_class", "category", "subcategory"]:
		if data.has(class_field) and not _safe_ui_string(data.get(class_field, "")).strip_edges().is_empty():
			class_tokens.append("%s=%s" % [class_field, _safe_ui_string(data.get(class_field, ""))])
	...
	identity.add_child(_create_property_row("Object class", class_label))
	v.add_child(identity)
	var current_status: VBoxContainer = _create_inspector_section("2. Current Status")
	var state_label: Label = Label.new(); state_label.text = _safe_ui_string(data.get("state", "(none)"), "(none)"); current_status.add_child(_create_property_row("state", state_label))
	for status_field in ["is_open", "is_closed", "is_locked", "is_powered", "damaged", "broken", "blocks_movement"]:
		if data.has(status_field):
			...
			status_value_label.text = _safe_ui_string(data.get(status_field, ""))
			current_status.add_child(_create_property_row(status_field, status_value_label))
	if entity_kind == "world_object" and type_group == "door" and mission_manager_runtime.has_method("get_map_constructor_door_visual_state"):
		var door_visual: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("get_map_constructor_door_visual_state", entity_id))
		var door_visual_label: Label = Label.new(); door_visual_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		door_visual_label.text = "visual=%s, badges=%s, note=%s" % [_safe_ui_string(door_visual.get("state", "unknown"), "unknown"), _safe_ui_string(door_visual.get("badges", [])), _safe_ui_string(door_visual.get("message", ""))]
		current_status.add_child(_create_property_row("Door visual", door_visual_label))
	if entity_kind == "world_object" and type_group == "terminal" and mission_manager_runtime.has_method("get_map_constructor_terminal_visual_state"):
		var terminal_visual: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("get_map_constructor_terminal_visual_state", entity_id))
		var terminal_visual_label: Label = Label.new(); terminal_visual_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		terminal_visual_label.text = "type=%s, state=%s, badges=%s" % [_safe_ui_string(terminal_visual.get("terminal_type", "unknown"), "unknown"), _safe_ui_string(terminal_visual.get("state", "unknown"), "unknown"), _safe_ui_string(terminal_visual.get("badges", []))]
		current_status.add_child(_create_property_row("Terminal visual", terminal_visual_label))
	v.add_child(current_status)
	var placement := _create_inspector_section("3. Placement")
	...
	move_button.pressed.connect(func() -> void:
		...
		if target_cell.x < 0 or target_cell.y < 0:
			show_hint("Move target must be a valid map cell.")
			return
		if mission_manager_runtime != null and mission_manager_runtime.has_method("move_map_constructor_entity_to_cell"):
			var r: Dictionary = mission_manager_runtime.call("move_map_constructor_entity_to_cell", entity_kind, entity_id, target_cell)
	...
	dup_button.pressed.connect(func() -> void:
		...
		if duplicate_cell.x < 0 or duplicate_cell.y < 0:
			show_hint("Duplicate target must be a valid map cell.")
			return
		if mission_manager_runtime != null and mission_manager_runtime.has_method("duplicate_map_constructor_entity_to_cell"):
			var r: Dictionary = mission_manager_runtime.call("duplicate_map_constructor_entity_to_cell", entity_kind, entity_id, duplicate_cell)
	...
	var configurable: VBoxContainer = _create_inspector_section("4. Configurable Parameters")
	_add_preset_buttons(configurable, entity_kind, entity_id)
	_add_map_constructor_active_settings(configurable, entity_kind, entity_id, data, type_group)
	if type_group == "control" or data.has("requires_external_control"):
		_add_bool_property(configurable, "requires_external_control", entity_kind, entity_id, "requires_external_control", data.get("requires_external_control", false))
	if data.has("state") and not (type_group == "door" or (type_group == "power" and _safe_ui_string(data.get("object_type", "")).to_lower().begins_with("power_source"))):
		_add_text_property(configurable, "Editable state override", entity_kind, entity_id, "state", data.get("state", ""))
	if type_group == "terminal":
		_add_bool_property(configurable, "damaged", entity_kind, entity_id, "damaged", data.get("damaged", false))
		_add_bool_property(configurable, "encrypted", entity_kind, entity_id, "encrypted", data.get("encrypted", false))
	if type_group == "power":
		var power_object_type: String = _safe_ui_string(data.get("object_type", "")).to_lower()
		if power_object_type.begins_with("power_source"):
			_add_enum_property(configurable, "Source state", entity_kind, entity_id, "state", data.get("state", "on"), [{"label":"On", "value":"on"}, {"label":"Off", "value":"off"}, {"label":"Damaged", "value":"damaged"}, {"label":"Broken", "value":"broken"}])
			_add_enum_property(configurable, "Source class", entity_kind, entity_id, "power_source_class", data.get("power_source_class", 1), [{"label":"Class 1 (4 outlets)", "value":"1"}, {"label":"Class 2 (5 outlets)", "value":"2"}, {"label":"Class 3 (6 outlets)", "value":"3"}])
		elif power_object_type == "power_cable" or power_object_type == "power_cable_reel":
			_add_enum_property(configurable, "Wire state", entity_kind, entity_id, "state", data.get("state", "ok"), [{"label":"OK", "value":"ok"}, {"label":"Damaged", "value":"damaged"}, {"label":"Broken", "value":"broken"}])
			_add_bool_property(configurable, "Hidden installation", entity_kind, entity_id, "is_hidden", data.get("is_hidden", false))
			_add_enum_property(configurable, "Route surface", entity_kind, entity_id, "route_surface", data.get("route_surface", "floor"), [{"label":"Floor", "value":"floor"}, {"label":"Wall", "value":"wall"}])
		elif power_object_type == "light":
			_add_text_property(configurable, "Brightness", entity_kind, entity_id, "brightness", data.get("brightness", "1.0"))
			_add_text_property(configurable, "Color", entity_kind, entity_id, "color", data.get("color", "#ffffff"))
		_add_bool_property(configurable, "is_powered", entity_kind, entity_id, "is_powered", data.get("is_powered", false))
		_add_bool_property(configurable, "damaged", entity_kind, entity_id, "damaged", data.get("damaged", false))
		_add_bool_property(configurable, "broken", entity_kind, entity_id, "broken", data.get("broken", false))
	if type_group == "item":
		var item_type_label: Label = Label.new()
		item_type_label.text = _safe_ui_string(data.get("item_type", data.get("object_type", "item")), "item")
		configurable.add_child(_create_property_row("Item type", item_type_label))
		if data.has("digital_payload_type") or _safe_ui_string(data.get("item_type", "")).findn("digital") >= 0 or _safe_ui_string(data.get("item_type", "")).findn("access_code") >= 0:
			_add_text_property(configurable, "digital_payload_type", entity_kind, entity_id, "digital_payload_type", data.get("digital_payload_type", ""))
	...
	var link_section: VBoxContainer = _create_inspector_section("5. Links")
	if _is_map_constructor_key_item(data, type_group):
		_add_key_door_link_section(link_section, entity_kind, entity_id, data)
	if type_group == "door":
		_add_door_linked_key_section(link_section, entity_id, data)
	if type_group == "terminal":
		_add_link_picker(link_section, entity_kind, entity_id, "linked_door", "Linked Door")
		_add_link_picker(link_section, entity_kind, entity_id, "terminal_target", "Terminal Target")
	if type_group == "power":
		_add_link_picker(link_section, entity_kind, entity_id, "power_source", "Linked Power Source")
		_add_link_picker(link_section, entity_kind, entity_id, "power_network", "Power Network")
	...
	if control_visible:
		_add_link_picker(link_section, entity_kind, entity_id, "control_source", "Control Source")
		_add_link_picker(link_section, entity_kind, entity_id, "linked_door", "Linked Door")
		_add_link_picker(link_section, entity_kind, entity_id, "terminal_target", "Terminal Target")
		_add_link_picker(link_section, entity_kind, entity_id, "platform_target", "Platform Target")
	...
	if floor_target_cell.x >= 0 and floor_target_cell.y >= 0:
		...
		if mission_manager_runtime != null and mission_manager_runtime.has_method("get_map_constructor_floor_material"):
			var current_floor_override: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("get_map_constructor_floor_material", floor_target_cell))
			selected_floor_material_id = _safe_ui_string(_safe_ui_dictionary(current_floor_override.get("override", {})).get("material_id", "steel_default"), "steel_default")
		...
		apply_floor_button.pressed.connect(func() -> void:
			...
			var floor_apply_result: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("set_map_constructor_floor_material", floor_target_cell, floor_material_id_apply))
		...
		clear_floor_button.pressed.connect(func() -> void:
			if mission_manager_runtime == null or not mission_manager_runtime.has_method("clear_map_constructor_floor_material"):
				return
			var floor_clear_result: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("clear_map_constructor_floor_material", floor_target_cell))
	...
	var wall_target: Dictionary = _resolve_wall_material_target_for_selection(entity_info, data, cell)
	if type_group != "item" and (type_group == "wall" or _safe_ui_string(data.get("placement_mode", "")) == "wall_mounted" or not selected_map_constructor_wall_side.is_empty()):
		...
		if bool(wall_target.get("ok", false)):
			...
			if mission_manager_runtime != null and mission_manager_runtime.has_method("get_map_constructor_wall_material_catalog"):
				catalog_result = _safe_ui_dictionary(mission_manager_runtime.call("get_map_constructor_wall_material_catalog"))
			var material_option: OptionButton = OptionButton.new()
			var material_rows: Array = _safe_ui_array(catalog_result.get("materials", []))
			var selected_material_id: String = ""
			if mission_manager_runtime != null and mission_manager_runtime.has_method("get_map_constructor_wall_material"):
				var current_override: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("get_map_constructor_wall_material", wall_cell, wall_side))
				selected_material_id = _safe_ui_string(_safe_ui_dictionary(current_override.get("override", {})).get("material_id", ""))
			var description_label: Label = Label.new()
			for row_variant in material_rows:
				var material_row: Dictionary = _safe_ui_dictionary(row_variant)
			...
			material_option.item_selected.connect(func(_idx: int) -> void:
				...
				for row_variant_inner in material_rows:
					var material_row_inner: Dictionary = _safe_ui_dictionary(row_variant_inner)
			...
			apply_material.pressed.connect(func() -> void:
				if mission_manager_runtime == null or not mission_manager_runtime.has_method("set_map_constructor_wall_material"):
					show_hint("Wall material action unavailable.")
					return
				var mat_id: String = _safe_ui_string(material_option.get_selected_metadata())
				var apply_result: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("set_map_constructor_wall_material", wall_cell, wall_side, mat_id))
			...
			clear_material.pressed.connect(func() -> void:
				if mission_manager_runtime == null or not mission_manager_runtime.has_method("clear_map_constructor_wall_material"):
					show_hint("Wall material action unavailable.")
					return
				var clear_result: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("clear_map_constructor_wall_material", wall_cell, wall_side))
	...
	if _safe_ui_string(data.get("placement_mode", "")) == "wall_mounted" and mission_manager_runtime != null and mission_manager_runtime.has_method("get_map_constructor_wall_mounted_status"):
		...
		deferred_wall_section.add_child(_create_property_row("attached_wall_cell", attached_label))
		var wall_side_picker: Control = _create_map_constructor_wall_side_picker("wall_mounted")
		deferred_wall_section.add_child(_create_property_row("wall_side", wall_side_picker))
		var apply_side: Button = Button.new(); apply_side.text = "Apply Side"
		apply_side.pressed.connect(func() -> void:
			if selected_map_constructor_wall_side.strip_edges().is_empty():
				show_hint("Select a valid wall side before applying.")
				return
			if mission_manager_runtime == null or not mission_manager_runtime.has_method("set_map_constructor_wall_mounted_side"):
				show_hint("Wall-mounted side action unavailable.")
				return
			var rr: Dictionary = _safe_ui_dictionary(mission_manager_runtime.call("set_map_constructor_wall_mounted_side", entity_kind, entity_id, selected_map_constructor_wall_side))
	...
	if deferred_wall_section != null:
		v.add_child(deferred_wall_section)
