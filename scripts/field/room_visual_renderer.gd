...
func _get_wall_cable_axis_dirs(wall_side: String) -> Array[String]:
	if wall_side == "se":
		return ["west", "east"]
	return ["south", "north"]

func _get_wall_cable_side_dirs(wall_side: String) -> Dictionary:
	var axis_dirs: Array[String] = _get_wall_cable_axis_dirs(wall_side)
	return {"prev": axis_dirs[0], "next": axis_dirs[1]}
...
func _get_wall_cable_rail_segment(cell: Vector2i, side: String) -> Dictionary:
	...
	return {"center": center, "start": center + Vector2(-half_width, 0.0), "end": center + Vector2(half_width, 0.0), "normal": Vector2(0.0, -1.0)}

func _get_wall_cable_face_cell(object_data: Dictionary, fallback_cell: Vector2i) -> Vector2i:
...
func _get_wall_cable_tap_anchor(object_data: Dictionary) -> Vector2:
	var cell: Vector2i = _get_cable_object_cell(object_data)
	if cell.x < 0 or cell.y < 0:
		return Vector2.ZERO
	var side: String = get_cable_wall_side(object_data)
	var face_cell: Vector2i = _get_wall_cable_face_cell(object_data, cell)
	var rail_anchor: Vector2 = _get_wall_cable_rail_anchor(face_cell, side)
...
func _should_draw_wall_mounted_cable_tap(object_data: Dictionary, has_terminal_visual: bool = false) -> bool:
	...
	if object_type.contains("socket") or object_type.contains("switch") or object_type.contains("fuse_box") or object_type.contains("button"):
		return true
	return false
...
func _get_wall_cable_face_run_span_key(face_cell: Vector2i, wall_side: String) -> String:
	if face_cell.x < 0 or face_cell.y < 0:
		return "invalid"
	var side_dirs: Dictionary = _get_wall_cable_side_dirs(wall_side)
	var prev_delta: Vector2i = _get_wall_side_delta(str(side_dirs.get("prev", "")))
	var next_delta: Vector2i = _get_wall_side_delta(str(side_dirs.get("next", "")))
	var run_start: Vector2i = face_cell
	while prev_delta != Vector2i.ZERO and _cell_has_wall_for_iso_cable(run_start + prev_delta):
		run_start += prev_delta
	var run_end: Vector2i = face_cell
	while next_delta != Vector2i.ZERO and _cell_has_wall_for_iso_cable(run_end + next_delta):
		run_end += next_delta
	return "%d,%d-%d,%d" % [run_start.x, run_start.y, run_end.x, run_end.y]

func _get_wall_cable_face_run_key(object_data: Dictionary, cell: Vector2i) -> String:
	var face_cell: Vector2i = _get_wall_cable_face_cell(object_data, cell)
	var wall_side: String = get_cable_wall_side(object_data)
	var routing_mode: String = get_cable_wall_routing_mode(object_data)
	var plane_key: String = _get_wall_cable_face_run_span_key(face_cell, wall_side)
...
func _get_wall_cable_anchor_sort_coordinate(anchor: Dictionary) -> float:
	var wall_side: String = str(anchor.get("wall_side", "sw")).strip_edges().to_lower()
	var rail_anchor: Vector2 = Vector2(anchor.get("rail_anchor", Vector2.ZERO))
	var axis: Vector2 = _get_wall_cable_visual_axis_for_side(wall_side)

	return rail_anchor.dot(axis)

func _insert_sorted_wall_cable_anchor(anchors: Array[Dictionary], anchor: Dictionary) -> void:
	var anchor_coord: float = _get_wall_cable_anchor_sort_coordinate(anchor)
	for index in range(anchors.size()):
		if anchor_coord < _get_wall_cable_anchor_sort_coordinate(anchors[index]):
			anchors.insert(index, anchor)
			return
	anchors.append(anchor)

func _get_wall_cable_object_id(object_data: Dictionary, cell: Vector2i) -> String:
	var object_id: String = str(object_data.get("id", object_data.get("object_id", ""))).strip_edges()
...
func _build_wall_cable_face_anchor(object_data: Dictionary, cell: Vector2i, base_wall_side: String, base_routing_mode: String) -> Dictionary:
	...
	if not is_cable and not should_draw_tap:
		return {}
	if get_cable_wall_side(object_data) != base_wall_side:
		return {}
	if get_cable_wall_routing_mode(object_data) != base_routing_mode:
		return {}
	var face_cell: Vector2i = _get_wall_cable_face_cell(object_data, cell)
	if face_cell.x < 0 or face_cell.y < 0:
		return {}
	var rail_anchor: Vector2 = _get_wall_cable_rail_anchor(face_cell, base_wall_side)
	var visual_center: Vector2 = get_wall_mounted_visual_center(object_data, cell) if is_wall_mounted_runtime_object(object_data) else grid_to_iso(cell)
	if should_draw_tap:
		rail_anchor = Vector2(visual_center.x, rail_anchor.y)
	return {
		"cell": face_cell,
		"object_id": _get_wall_cable_object_id(object_data, cell),
		"object_type": str(object_data.get("object_type", object_data.get("type", ""))),
		"rail_anchor": rail_anchor,
		"visual_center": visual_center,
		"circuit_id": CableTopologyServiceRef.get_cable_circuit_id(object_data),
		"wall_side": base_wall_side,
		"routing_mode": base_routing_mode,
		"is_cable": is_cable,
		"should_draw_tap": should_draw_tap,
		"object_data": object_data
	}

func _collect_wall_cable_face_run_anchors(base_data: Dictionary, base_cell: Vector2i) -> Array[Dictionary]:
...
func _get_wall_cable_anchor_face_run_key(anchor: Dictionary) -> String:
	var wall_side: String = str(anchor.get("wall_side", "")).strip_edges().to_lower()
	var routing_mode: String = str(anchor.get("routing_mode", "outer")).strip_edges().to_lower()
	var rail_anchor: Vector2 = Vector2(anchor.get("rail_anchor", Vector2.ZERO))
	...
	var axis: Vector2 = _get_wall_cable_visual_axis_for_side(wall_side)
	var normal: Vector2 = Vector2(-axis.y, axis.x).normalized()

	# Группируем не по wall-cell span, а по экранной линии кабеля.
	# Это убирает зависимость от _cell_has_wall_for_iso_cable(),
	# из-за которой кабели на одной видимой стене попадали в разные группы.
	var line_bucket_size: float = 16.0
	var line_bucket: int = int(round(rail_anchor.dot(normal) / line_bucket_size))

	return "%s|%s|line_%d" % [wall_side, routing_mode, line_bucket]
...
func _is_wall_cable_same_side_visible_span(anchor_a: Dictionary, anchor_b: Dictionary) -> bool:
	...
	var delta: Vector2 = rail_b - rail_a

	if delta.length_squared() <= 0.25:
		return false

	var axis: Vector2 = _get_wall_cable_visual_axis_for_side(side_a)
	var normal: Vector2 = Vector2(-axis.y, axis.x).normalized()

	var axis_distance: float = absf(delta.dot(axis))
	var normal_distance: float = absf(delta.dot(normal))

	var half_size: Vector2 = get_iso_tile_half_size()

	# Соседи на одной видимой плоскости стены могут немного плавать по normal
	# из-за wall-mounted center / attached_wall_cell, но не должны быть на другой линии.
	var max_normal_error: float = maxf(half_size.y * 0.95, 24.0)

	# Это расстояние до ближайшей следующей ячейки вдоль стены.
	# Оно не "растягивает линию", а только отсекает слишком дальние разрывы.
	var max_axis_step: float = maxf(half_size.x * 2.35, 120.0)

	if normal_distance > max_normal_error:
		return false

	if axis_distance > max_axis_step:
		return false
...
func _draw_wall_cable_side_anchor_segments_clipped(side_anchors: Array, all_circuit_anchors: Array, profile: Dictionary) -> void:
	...
	for anchor_variant in cable_anchors:
		var anchor_a: Dictionary = Dictionary(anchor_variant)
		...
		var best_axis_distance: float = INF

		for candidate_variant in cable_anchors:
			var anchor_b: Dictionary = Dictionary(candidate_variant)
			...
			var rail_b: Vector2 = _get_wall_cable_anchor_rail(anchor_b)
			var axis_distance: float = (rail_b - rail_a).dot(axis)

			# Ищем только следующего соседа вперед по линии стены.
			# Обратное направление нарисует предыдущий anchor.
			if axis_distance <= 0.25:
				continue

			if axis_distance < best_axis_distance:
				best_axis_distance = axis_distance
				best_anchor = anchor_b

		if best_anchor.is_empty():
			continue

		var id_b_final: String = _get_wall_cable_anchor_id(best_anchor)
		var pair_ids: Array[String] = [id_a, id_b_final]
		pair_ids.sort()
		...
		drawn_pair_keys[pair_key] = true
		connected_anchor_ids[id_a] = true
		connected_anchor_ids[id_b_final] = true

		_draw_visible_wall_cable_span_between_anchors(anchor_a, best_anchor, profile)
...
func _build_wall_cable_visual_polyline(path_data: Dictionary) -> PackedVector2Array:
	...
	var anchors: Array[Dictionary] = Array(path_data.get("anchors", []))
	...
	if anchors.size() >= 2:
		points.append(Vector2(Dictionary(anchors[0]).get("rail_anchor", Vector2.ZERO)))
		points.append(Vector2(Dictionary(anchors[anchors.size() - 1]).get("rail_anchor", Vector2.ZERO)))
...
func _draw_wall_cable_run_graphics(run_cells: Array[Vector2i], object_data: Dictionary, profile: Dictionary, topology: Dictionary, head_cell: Vector2i, anchors: Array[Dictionary] = []) -> void:
	...
	# Основной draw path должен работать по сторонам стены, а не по face_run_groups.
	# face_run_groups зависит от wall span key / line bucket и дробит нормальную цепь.
	var side_groups: Dictionary = _partition_wall_cable_anchors_by_side(anchors)

	_draw_wall_cable_side_anchor_segments_clipped(
		Array(side_groups.get("sw", [])),
		anchors,
		profile
	)

	_draw_wall_cable_side_anchor_segments_clipped(
		Array(side_groups.get("se", [])),
		anchors,
		profile
	)

	# SW <-> SE угловые переходы рисуем отдельно после прямых линий.
...
func _get_wall_cable_neighbors(cell: Vector2i, object_data: Dictionary) -> Dictionary:
	var result: Dictionary = {}
	var world_objects: Array[Dictionary] = _get_runtime_world_objects_for_iso_render(true)
	world_objects.append(object_data)
	var circuit_id: String = CableTopologyServiceRef.get_cable_circuit_id(object_data)
	for neighbor_object in world_objects:
		if not (neighbor_object is Dictionary):
			continue
		var neighbor_data: Dictionary = Dictionary(neighbor_object)
		if _get_wall_routed_object_family(neighbor_data) != "cable":
			continue
		var neighbor_cell: Vector2i = _get_cable_object_cell(neighbor_data)
		if neighbor_cell.x < 0 or neighbor_cell.y < 0 or neighbor_cell == cell:
			continue
		var delta: Vector2i = neighbor_cell - cell
		...
		if delta == Vector2i(0, -1):
			...
			continue
		if not circuit_id.is_empty() and CableTopologyServiceRef.get_cable_circuit_id(neighbor_data) != circuit_id:
			continue
		if not result.has(direction):
			result[direction] = []
		Array(result[direction]).append(neighbor_data)
	return result

func classify_wall_cable_topology(cell: Vector2i, object_data: Dictionary) -> Dictionary:
	var wall_side: String = get_cable_wall_side(object_data)
	var routing_mode: String = get_cable_wall_routing_mode(object_data)
	var side_dirs: Dictionary = _get_wall_cable_side_dirs(wall_side)
	var axis_dirs: Array[String] = [str(side_dirs.get("prev", "")), str(side_dirs.get("next", ""))]
	var connected_dirs: Dictionary = {"north": false, "south": false, "west": false, "east": false}
	var floor_transition_dirs: Dictionary = {}
	var corner_dirs: Dictionary = {}
	var neighbor_cells: Array[Vector2i] = []
	var neighbors: Dictionary = _get_wall_cable_neighbors(cell, object_data)
	for direction in ["north", "south", "west", "east"]:
		for neighbor_data_variant in Array(neighbors.get(direction, [])):
			var neighbor_data: Dictionary = Dictionary(neighbor_data_variant)
			var neighbor_cell: Vector2i = _get_cable_object_cell(neighbor_data)
			if is_wall_cable_object(neighbor_data):
				...
				if axis_dirs.has(direction):
					...
					corner_dirs[direction] = true
				neighbor_cells.append(neighbor_cell)
			elif is_floor_cable_object(neighbor_data) or is_hidden_floor_cable_object(neighbor_data):
				var transition_mode: String = "floor_to_wall_outer"
				if is_hidden_floor_cable_object(neighbor_data) and routing_mode == "inner":
					transition_mode = "hidden_to_wall_inner"
				elif is_hidden_floor_cable_object(neighbor_data):
					transition_mode = "hidden_to_wall_outer"
				elif routing_mode == "inner":
					transition_mode = "floor_to_wall_inner"
				floor_transition_dirs[direction] = transition_mode
				neighbor_cells.append(neighbor_cell)
	return {
		"wall_side": wall_side,
		"routing_mode": routing_mode,
		"connect_prev": bool(connected_dirs.get(str(side_dirs.get("prev", "")), false)),
		"connect_next": bool(connected_dirs.get(str(side_dirs.get("next", "")), false)),
		"connect_floor": not floor_transition_dirs.is_empty(),
		"connect_corner": not corner_dirs.is_empty(),
		"floor_transition_mode": "none" if floor_transition_dirs.is_empty() else str(floor_transition_dirs.values()[0]),
		"connected_dirs": connected_dirs,
		"floor_transition_dirs": floor_transition_dirs,
...
func get_wall_cable_axis_points(visual_center: Vector2, object_data: Dictionary) -> Dictionary:
	...
	var center: Vector2 = start_edge.lerp(end_edge, 0.5)
	...
	var base: Vector2 = center + Vector2(0.0, get_wall_mount_height_screen_px(source_height_px))
	return {"center": center, "start_edge": start_edge, "end_edge": end_edge, "axis": axis, "normal": normal, "base": base}

func _get_wall_cable_endpoint(_cell: Vector2i, visual_center: Vector2, object_data: Dictionary, direction: String) -> Vector2:
	var axis_points: Dictionary = get_wall_cable_axis_points(visual_center, object_data)
	var wall_side: String = get_cable_wall_side(object_data)
	var side_dirs: Dictionary = _get_wall_cable_side_dirs(wall_side)
	if direction == str(side_dirs.get("prev", "")):
		return Vector2(axis_points.get("start_edge", visual_center))
	if direction == str(side_dirs.get("next", "")):
		return Vector2(axis_points.get("end_edge", visual_center))
	var floor_hint: Vector2 = grid_to_iso(_cell)
	if direction in ["north", "south", "east", "west"]:
		floor_hint = get_iso_cable_branch_endpoint(_cell, direction)
	var start_edge: Vector2 = Vector2(axis_points.get("start_edge", visual_center))
	var end_edge: Vector2 = Vector2(axis_points.get("end_edge", visual_center))
	return start_edge if start_edge.distance_squared_to(floor_hint) <= end_edge.distance_squared_to(floor_hint) else end_edge

func _draw_wall_cable_inner_hint(center: Vector2, normal: Vector2) -> void:
	draw_line(center - normal * 5.0, center + normal * 5.0, Color(0.01, 0.012, 0.016, 0.72), 4.0, true)
	draw_line(center - normal * 3.0, center + normal * 3.0, Color(0.70, 0.74, 0.78, 0.25), 1.1, true)

func _draw_wall_cable_transition(cell: Vector2i, visual_center: Vector2, object_data: Dictionary, direction: String, transition_mode: String, profile: Dictionary) -> void:
	var wall_side: String = get_cable_wall_side(object_data)
	var rail_segment: Dictionary = _get_wall_cable_rail_segment(cell, wall_side)
	var rail_anchor: Vector2 = Vector2(rail_segment.get("center", visual_center))
	var floor_endpoint: Vector2 = get_iso_cable_branch_endpoint(cell, direction)
	var wall_foot: Vector2 = Vector2(rail_anchor.x, floor_endpoint.y)
	var normal: Vector2 = Vector2(rail_segment.get("normal", profile.get("wall_normal", Vector2.UP))).normalized()
	if normal == Vector2.ZERO:
		normal = Vector2.UP
	match transition_mode:
		"floor_to_wall_inner":
			draw_iso_cable_mode_segment(floor_endpoint, wall_foot, profile)
			draw_iso_cable_mode_segment(wall_foot, rail_anchor, profile)
			_draw_wall_cable_inner_hint(rail_anchor, normal)
		"hidden_to_wall_outer":
			_draw_wall_cable_inner_hint(wall_foot, normal)
			draw_iso_cable_hidden_segment(floor_endpoint, wall_foot, profile)
			draw_iso_cable_hidden_segment(wall_foot, rail_anchor, profile)
		"hidden_to_wall_inner":
			_draw_wall_cable_inner_hint(wall_foot, normal)
		_:
			draw_iso_cable_mode_segment(floor_endpoint, wall_foot, profile)
			draw_iso_cable_mode_segment(wall_foot, rail_anchor, profile)
